...
func emailVerifiedCallback(ctx context.Context, mt *workflow.Run[MyType, status], r io.Reader) (status, error) {
	...
	mt.Object.Email = "test@example.com"
...
func TestTimeTimerFunc(t *testing.T) {
	...
	runID, err := wf.Trigger(ctx, "test-user")
	require.NoError(t, err)

	workflow.AwaitTimeoutInsert(t, wf, "test-user", runID, StatusStart)
	...
	workflow.Require(t, wf, "test-user", StatusEnd, expected)
}
